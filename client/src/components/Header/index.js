...
import "./Header.css"
import logo from '../../assets/img/us-logo.png'
		...
		<>
			<header className={!isMenuOpen && isScrolled ? "header scrolled" : "header"} id='headerID'>
				<Link to='/' className='header__logoWrapper' onClick={() => setIsMenuOpen(false)}>
					<img src={logo} alt='Utkarsh Singh' />
				</Link>
				<div className='header__nameWrapper'>
					<p className='header__name'>
						{page ? (
							<>
								<span className='responsive__hide'>Utkarsh's</span> <span className='pageName'>{page}</span>
							</>
						) : (
							<span className='responsive__hide'>Utkarsh Singh</span>
						)}
					</p>
					{!page && (
						<p className='header__profession'>
							<span className='responsive__hide'>B.Tech CSE Student & Full-Stack Developer</span>
						</p>
					)}
				</div>
